...
public class CustomerDao
{
    ...
    public PageBean<Customer> findAll(int pc, int pr)
    {
        try{
            ...
            sql="select * from table_1 order by name limit ?,?";
    ...
    {
        try{
            String sql="update table_1 set name=?,gender=?,phone=?,email=?,description=? where id=?";
    ...
    public PageBean<Customer> query(Customer customer,int pc,int pr) {
        try {
            ...
            StringBuilder cntSql = new StringBuilder("select count(*) from table_1 ");
            ...
            StringBuilder sql=new StringBuilder("select * from table_1 ");
